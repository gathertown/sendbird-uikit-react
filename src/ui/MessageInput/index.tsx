...
const MessageInput = React.forwardRef<HTMLInputElement, MessageInputProps>((props, externalRef) => {
  ...
  useEffect(() => {
    ...
    const editMessageOrDraftMessage = message ?? draftMessage;
    if (editMessageOrDraftMessage) {
      // const textField = document.getElementById(textFieldId);
      const textField = internalRef?.current;
      if (isMentionEnabled && editMessageOrDraftMessage?.mentionedUsers?.length > 0 && editMessageOrDraftMessage?.mentionedMessageTemplate?.length > 0) {
        /* mention enabled */
        const { mentionedUsers = [] } = editMessageOrDraftMessage;
        const tokens = tokenizeMessage({
          messageText: editMessageOrDraftMessage?.mentionedMessageTemplate,
          mentionedUsers,
        });
        ...
        /* mention disabled */
        try {
          textField.innerHTML = sanitizeString(editMessageOrDraftMessage.message);
        } catch {
          //
        }
        setMentionedUserIds([]);
